...
# install typst-ts. It just does platform detection, downloads the installer
# and runs it.

set -u

UPDATE_ROOT="https://github.com/Myriad-Dreamin/typst.ts/releases/download/$VERSION"
...
main() {
    ...
    local _tardir="typst-ts-${_arch}"
    local _url="$UPDATE_ROOT/${_tardir}.tar.gz"
    local _dir="$(mktemp -d 2>/dev/null || ensure mktemp -d -t typst-ts)"
    local _file="$_dir/input.tar.gz"
    local _wasmpack="$_dir/typst-ts$_ext"
    local _wasmpackinit="$_dir/typst-ts-init$_ext"

    printf '%s\n' 'info: downloading typst-ts' 1>&2
    ...
    if [ $? != 0 ]; then
      ...
      say "that typst-ts's release process is not working. When in doubt"
